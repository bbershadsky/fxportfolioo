...
export function SolanaChart({ className }: Props) {
  const { data, loading } = useSolanaData();
  ...
  return (
    ...
    >
      <CardHeader className="px-4">
        <CardTitle className="mt-1 text-base">Solana</CardTitle>
      </CardHeader>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData}>
              ...
              <Line
                type="monotone"
                dataKey="price"
                stroke="#9945FF" // Solana-specific color
                strokeWidth={1.5}
              />
            </LineChart>
          </ResponsiveContainer>
